<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h1>Créer une nouvelle alerte</h1>
      ...
      <div class="card-body">
        <form @submit.prevent="saveAlert">
          ...
          <div class="mb-3">
            ...
            <select id="type" v-model="alert.type" class="form-select" required @change="updateAlertSettings">
              <option value="" disabled>Sélectionnez un type d'alerte</option>
              <option value="budget_category">Alerte sur catégorie de budget</option>
              <option value="budget_total">Alerte sur budget total</option>
              <option value="account_balance">Alerte sur solde de compte</option>
            </select>
          </div>
          ...
          <div v-if="alert.type === 'budget_category'" class="row mb-3">
            <div class="col-md-6">
              ...
              <select id="budget_id" v-model="alert.budget_id" class="form-select" required @change="fetchBudgetCategories">
            ...
            <div class="col-md-6">
              ...
              <select id="categorie_budget_id" v-model="alert.categorie_budget_id" class="form-select" required :disabled="!alert.budget_id">
                <option value="" disabled>Sélectionnez une catégorie</option>
                <option v-for="category in budgetCategories" :key="category.id" :value="category.id">
                  {{ category.nom }} ({{ formatAmount(category.montant_alloue) }})
                </option>
              </select>
            </div>
          </div>
          ...
          <div v-if="alert.type === 'budget_total'" class="mb-3">
            <label for="budget_id_total" class="form-label">Budget</label>
            <select id="budget_id_total" v-model="alert.budget_id" class="form-select" required>
              <option value="" disabled>Sélectionnez un budget</option>
              <option v-for="budget in budgets" :key="budget.id" :value="budget.id">
                {{ budget.nom }} ({{ formatAmount(budget.montant_total) }})
              </option>
            </select>
          </div>
          ...
          <div v-if="alert.type === 'account_balance'" class="mb-3">
            ...
            <select id="account_id" v-model="alert.account_id" class="form-select" required>
          ...
          <div class="d-flex justify-content-between">
            ...
            <button type="submit" class="btn btn-primary">
              <i class="fas fa-save me-1"></i> Créer l'alerte
            </button>
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    ...
    fetchBudgetCategories() {
      if (!this.alert.budget_id) return;
      
      this.alert.categorie_budget_id = '';
      
      axios.get(`/api/user/budgets/${this.alert.budget_id}/categories`)
        .then(response => {
    ...
    updateAlertSettings() {
      ...
      this.alert.budget_id = '';
      this.alert.categorie_budget_id = '';
      this.alert.account_id = '';
    ...
    validateForm() {
      ...
      if (!this.alert.type) {
        this.errors.push('Le type d\'alerte est requis.');
        return false;
      }

      if (this.alert.type === 'budget_category' && (!this.alert.budget_id || !this.alert.categorie_budget_id)) {
        this.errors.push('Veuillez sélectionner un budget et une catégorie.');
        return false;
      }

      if (this.alert.type === 'budget_total' && !this.alert.budget_id) {
        this.errors.push('Veuillez sélectionner un budget.');
        return false;
      }

      if (this.alert.type === 'account_balance' && !this.alert.account_id) {
    ...
    saveAlert() {
      ...
      // Ajouter les champs spécifiques selon le type d'alerte
      if (this.alert.type === 'budget_category') {
        alertData.budget_id = this.alert.budget_id;
        alertData.categorie_budget_id = this.alert.categorie_budget_id;
      } else if (this.alert.type === 'budget_total') {
        alertData.budget_id = this.alert.budget_id;
      } else if (this.alert.type === 'account_balance') {
        alertData.account_id = this.alert.account_id;
      }

      // Ajouter le seuil selon la condition
      if (this.alert.condition === 'percentage') {
        alertData.threshold = this.alert.percentage_threshold;
      } else {
        alertData.threshold = this.alert.amount_threshold;
      }

      axios.post('/api/user/alerts', alertData)
        .then(response => {
          this.$toasted.success('Alerte créée avec succès!');
        ...
        .catch(error => {
          if (error.response && error.response.data && error.response.data.errors) {
            ...
            this.errors = ['Une erreur est survenue lors de la création de l\'alerte.'];
          }
        });
    }
  },
  ...
  metaInfo() {
    return {
      title: 'Créer une alerte - Budget Manager'
    };
  }
};
</script>